...
import { RemoveJobSkill } from "./RemoveJobSkill";
...
describe('RemoveJobSkill UseCase', () => {
    ...
    beforeEach(() => {
        ...
        removeJobSkill = new RemoveJobSkill(mockJobRepository, mockSkillRepository);
    ...
    it('should remove an existing skill from job', async () => {
        ...
        await removeJobSkill.execute({ jobId: 1, skillId: 10, type: 'mandatory' });

        expect(mockJobRepository.findById).toHaveBeenCalledWith(1);
        expect(mockSkillRepository.findById).toHaveBeenCalledWith(10);
        expect(mockJobRepository.removeSkill).toHaveBeenCalledWith(1, 10, 'mandatory');
    });

    it('should do nothing if skill does not exist', async () => {
        mockJobRepository.findById.mockResolvedValue(existingJob);
        mockSkillRepository.findById.mockResolvedValue(null);

        await removeJobSkill.execute({ jobId: 1, skillId: 99, type: 'recommended' });

        expect(mockJobRepository.findById).toHaveBeenCalledWith(1);
        expect(mockSkillRepository.findById).toHaveBeenCalledWith(99);
        expect(mockJobRepository.removeSkill).not.toHaveBeenCalled();
    });

    it('should throw error if job does not exist', async () => {
        mockJobRepository.findById.mockResolvedValue(null);
        await expect(removeJobSkill.execute({ jobId: 99, skillId: 10, type: 'mandatory' }))
